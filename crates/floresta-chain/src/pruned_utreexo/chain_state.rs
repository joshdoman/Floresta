...
//! - [BlockWithInputsConsumer]: Trait for receiving new block notifications with input UTXOs
extern crate alloc;
...
/// Trait for components that need to receive notifications about new blocks and their inputs.
pub trait BlockWithInputsConsumer: Sync + Send + 'static {
    fn consume_block_with_inputs(
        &self,
        block: &Block,
        height: u32,
...
pub struct ChainStateInner<PersistedState: ChainStore> {
    ...
    input_subscribers: Vec<Arc<dyn BlockWithInputsConsumer>>,
...
impl<PersistedState: ChainStore> ChainState<PersistedState> {
    ...
    fn notify(&self, block: &Block, height: u32, inputs: HashMap<OutPoint, UtxoData>) {
        ...
        for client in &inner.input_subscribers {
            client.consume_block_with_inputs(block, height, &inputs);
        }
    }
